...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run EFFORT on sampled frames from one video.")
        ...
        top_frames.append(
            {
                ...
                "reason": "The wrapper reports this frame because it had the highest EFFORT score among the sampled frames.",
            }
        )
    ...
    with torch.no_grad():
        for start in range(0, len(sampled_frames), args.batch_size):
            ...
            scores.extend(
                ...
                for index in range(predictions["prob"].shape[0])
            )

    if not scores:
        fail(f"EFFORT did not produce any predictions for {video_path}.")

    score = float(np.mean(scores))
    details = build_details(sampled_frames, scores, video_info, max(args.frames, 1))
    print(f"EFFORT video fakeness: {score:.4f}")
